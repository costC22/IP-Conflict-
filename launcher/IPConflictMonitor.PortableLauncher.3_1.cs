...
[assembly: AssemblyDescription("Diagnostico portatil e nativo de conflitos IPv4/MAC")]
    ...
    internal static partial class Program
    {
        ...
        private static int Main(string[] args)
        {
            try
            {
                ...
                {
                    HideConsoleWindow();
                    UpdateCoordinator.RegisterInterfaceHook();
                    return RunGraphicalInterface();
                }
                if (HasSwitch(args, "-GuiScreenshot", "--gui-screenshot"))
                {
                    HideConsoleWindow();
                    UpdateCoordinator.RegisterInterfaceHook();
                    return CaptureGraphicalInterface(GetOptionValue(args, "-GuiScreenshot", "--gui-screenshot"));
                }
                if (HasSwitch(args, "-Help", "--help", "/?", "-?")) { PrintHelp(); return 0; }
                if (HasSwitch(args, "-Status", "--status", "/Status")) { return ShowStatus(); }
                if (HasSwitch(args, "-Install", "--install", "/Install"))
                {
                    Console.Error.WriteLine("A instalacao persistente nao faz parte da edicao portatil.");
                    return 2;
                }
            ...
            {
                Console.Error.WriteLine("IPConflictMonitor: " + exception.Message);
                return 1;
            }
        }
        ...
        {
            string data = GetUserDataDirectory();
            string snapshot = Path.Combine(data, "reports", "snapshot.csv");
            Console.WriteLine("IPConflictMonitor 3.1 - Field Edition");
            Console.WriteLine("Motor: C# nativo; PowerShell nao utilizado");
        ...
        private static string EnsureDefaultConfiguration(string applicationDirectory)
        {
            ...
            if (!File.Exists(path)) { File.WriteAllText(path, ReadResourceText(ConfigResource), new UTF8Encoding(false)); }
            return path;
        }
        ...
        private static string ReadResourceText(string resourceName)
        {
            ...
            {
                if (stream == null) { throw new InvalidOperationException("Recurso interno ausente: " + resourceName); }
        ...
        private static string FindConfigArgument(string[] args)
        {
            ...
            {
                if (String.Equals(args[index], "-ConfigPath", StringComparison.OrdinalIgnoreCase) || String.Equals(args[index], "--ConfigPath", StringComparison.OrdinalIgnoreCase) || String.Equals(args[index], "/ConfigPath", StringComparison.OrdinalIgnoreCase)) { return index + 1 < args.Length ? args[index + 1] : String.Empty; }
            }
            return null;
        }
        ...
        private static string[] RemoveSwitches(string[] args, params string[] names)
        {
            ...
            {
                bool remove = false;
                foreach (string name in names) { if (String.Equals(argument, name, StringComparison.OrdinalIgnoreCase)) { remove = true; break; } }
                if (!remove) { output.Add(argument); }
            }
        ...
        {
            for (int index = 0; index < args.Length; index++) { foreach (string name in names) { if (String.Equals(args[index], name, StringComparison.OrdinalIgnoreCase)) { return index + 1 < args.Length ? args[index + 1] : null; } } }
            return null;
        }
        ...
        {
            Console.WriteLine("IPConflictMonitor 3.1.0 - Field Edition");
            ...
            Console.WriteLine();
            Console.WriteLine("Seguranca: sem PowerShell, sem autoelevacao, sem servico e sem tarefa agendada.");
        }
    }
}
